...
fn test_siphash() {
    ...
    let mut buf = Vec::new();
    let mut t = 0;
    let mut stream_inc = State::new(k0, k1);
    let mut stream_full = State::new(k0, k1);

    while t < 64 {
        let vec = u8to64_le!(vecs[t], 0);
        let out = Bytes(buf.as_slice()).hash(k0, k1);
        if vec != out { abort() }

        stream_full.reset();
        stream_full.write(buf.as_slice());
        let f = stream_full.result();
        let i = stream_inc.result();

        let v = u8to64_le!(vecs[t], 0);

        if f != i || f != v { abort() }

        buf.push(t as u8);
        stream_inc.write([t as u8]);
